...
document.querySelector('.wallet-button').addEventListener('click', async () => {
    try {
      if (Telegram.WebApp.initData) {
        // Telegram Wallet is available
        const wallet = Telegram.WebApp.initData;
        alert('Connected to Telegram Wallet');
      } else {
        alert('Telegram Wallet is not available. Please ensure you are in the Telegram app.');
      }
    } catch (error) {
      console.error('Error connecting to Telegram Wallet:', error);
      alert('Failed to connect to Telegram Wallet');
    }
  });
